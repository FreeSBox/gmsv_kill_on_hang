...
namespace memory
{
	module::module(const std::string_view name) :range(nullptr, 0), m_name(name), m_loaded(false), m_module(nullptr)
	{
		try_get_module();
	}
	...
	bool module::try_get_module()
	{
		...
		for( const std::string &search_path : SymbolFinderPaste::search_paths )
		{
			...
			m_module = dlopen( buffer, RTLD_LAZY | RTLD_NOLOAD );
			if( m_module != nullptr )
			{
				break;
			}
		...
		SymbolFinderPaste::GetLibraryInfo(m_module, lib);
